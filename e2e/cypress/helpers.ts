...
export function checkTreeView(tree: TreeViewNode) {
  ...
  function checkDescendants(
    name: string,
    node: TreeViewNode,
    getCurrentElement: () => Cypress.Chainable<JQuery<HTMLElement>>,
  ) {
    ...
    it(`should have same selected (${name})`, () => {
      getCurrentElement().find(".nodeText").first().should(
        node.selected ? "have.class" : "not.have.class",
    ...
    if (node.children == null) {
      ...
      for (let i = 0; i < node.children.length; i++) {
        checkDescendants(
          ...
          node.children[i],
          () => getTreeViewChildrenElement().children().eq(i),
        );
      }
    }
  }
